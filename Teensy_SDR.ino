...
const int8_t SEL0 = 0;   // USB/LSB
const int8_t SEL1 = 1;   // USB/LSB
const int8_t ModeSW = 2;   // USB/LSB
const int8_t BandSW = 3;   // band selector
...
void setup()
{
  ...
  */
  ...
  show_frequency((unsigned long)bands[STARTUP_BAND].freq);
  show_steps(steps[2].name);
  ...
  pinMode(SEL1, OUTPUT); // set sel0 and sel1 as ouputs
  pinMode(SEL0, OUTPUT);

  digitalWrite(SEL0, HIGH); // selects filter for 40M band
